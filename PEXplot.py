...
def Plot_compare_two(Filepath=None,Filepath2=None, rangee=None, cmap=0):  # rang=[start,end]
    ...
    plt.show()
'''
def Plot_Spacedata_gif(Filepath=None):  # rang=[start,end]TODO not implemented yet
    print(Filepath)
    if not os.path.isdir(join(Filepath,"gif")):
        os.mkdir(join(Filepath"gif"))
    File = np.array(helper.pload(Filepath), dtype=object)
    header=File[2]
    nv = list(header["sWiPMemBlock"]["adFree"].copy() )
    nv.append(0)
    for x in range(len(nv)):
        ...
        B1plus = np.array(np.abs(File[1]))[:,:,7,x,0]
        print(B1plus.shape)
        # data,0,x,y kspacedatanii, spacedata, header, maskch, B1parray, U90array, Im_Ch_array, AlphaU_full_array)



        plt.imshow( B1plus,vmax=0.2,vmin=0  , cmap='inferno')
        clb = plt.colorbar()
        clb.ax.tick_params(labelsize=8)
        plt.title(str(nv[x])+"Voltage")
        clb.ax.set_title(' μT/√(kW)', fontsize=8)
        plt.show()




        plt.savefig(join(Filepath,"gif",files_list[0][:-8]+str(x)+"Gif.png"), dpi=150)

    frames = [Image.open(image) for image in glob.glob(f"{join(Filepath,'gif')}/*.png")]
    frame_one = frames[0]
    frame_one.save("Spacedata.gif", format="GIF", append_images=frames,
               save_all=True, duration=50, loop=0)
        '''

    #for file in files_list:
    #    Plot_one(join(pathfolder,file))